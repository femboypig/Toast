    ...
    present: @escaping (ViewController, Any?) -> Void) -> GalleryItem?
{
    ...
    if let image = media as? TelegramMediaImage {
        if let file = image.video {
            let captureProtected = (UserDefaults.standard.object(forKey: "Toast_allowScreenshots") as? Bool ?? true) ? false : (message.isCopyProtected() || message.containsSecretMedia || message.minAutoremoveOrClearTimeout == viewOnceTimeout || message.paidContent != nil || peerIsCopyProtected)
